...
All distilled knowledge lives in `~/.nousphere/` by default:
...
Set `NOUSPHERE_HOME` in `.env` to change the data directory. For local dev, point it at `./output` (already gitignored):

```
NOUSPHERE_HOME=/Users/you/nousphere/output
```
...
node scripts/distill-one.js path/to/transcript.jsonl --save   # save to $NOUSPHERE_HOME/sessions/
```
...
```bash
node scripts/build-index.js              # reads from $NOUSPHERE_HOME/sessions/
